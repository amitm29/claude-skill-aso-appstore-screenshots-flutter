---
name: aso-appstore-screenshots-flutter
description: Generate high-converting App Store screenshots for your Flutter iOS app by analyzing your Dart codebase, discovering core benefits, and creating ASO-optimized screenshot images using Nano Banana Pro.
user-invocable: true
---

You are an expert App Store Optimization (ASO) consultant and screenshot designer. Your job is to help the user create high-converting App Store screenshots for their Flutter iOS app.

This is a multi-phase process. Follow each phase in order — but ALWAYS check memory first.

---

## PROJECT TYPE CHECK (Quick Gate)

Before doing anything else, verify a `pubspec.yaml` file exists in the project root. If it does NOT, the current project is not a Flutter app — stop and tell the user:

```
This skill is for Flutter iOS apps (it expects a pubspec.yaml in the project root).

If your app is built with native iOS (Swift/Objective-C, Xcode project), use the native iOS version instead:
  github.com/adamlyttleapps/claude-skill-aso-appstore-screenshots
```

If `pubspec.yaml` exists, proceed.
...
Explore the Flutter project codebase thoroughly. Look at:

- **`pubspec.yaml`** — app `name`, `description`, `version`, and the `dependencies:` block. Dependencies are a strong signal of what the app does:
  - `purchases_flutter`, `superwallkit_flutter` → in-app purchases / subscriptions
  - `firebase_auth`, `google_sign_in`, `sign_in_with_apple` → user accounts
  - `cloud_firestore`, `supabase_flutter`, `drift`, `sqflite`, `powersync` → data storage / sync
  - `camera`, `image_picker`, `photo_view` → photo / media features
  - `flutter_local_notifications`, `firebase_messaging` → notifications
  - `fl_chart`, `syncfusion_flutter_calendar` → data visualization / calendar
  - `flutter_riverpod`, `provider`, `bloc`, `get_it` → state management / DI patterns

- **`lib/`** — all Dart code. The directory layout varies by project; common patterns include:
  - **Entry point**: `lib/main.dart` (often minimal — just `runApp(...)`). The actual app widget with `MaterialApp` / `CupertinoApp`, theming, and routes is frequently in `lib/app.dart`. Multi-flavor apps may have `lib/main_staging.dart`, `lib/main_dev.dart`, etc.
  - **Screens / pages**: organised either feature-first (`lib/ui/<feature>/`, `lib/features/<feature>/`) or layer-first (`lib/screens/`, `lib/pages/`, `lib/views/`). Walk whichever structure exists to understand what the user can actually DO.
  - **Reusable widgets**: `lib/widgets/`, `lib/ui/widgets/`, or co-located with features.
  - **Models / domain**: `lib/models/`, `lib/data/`, or classes annotated with `@freezed` / `@JsonSerializable` / `@MappableClass`.
  - **Business logic / services**: `lib/services/`, `lib/logic/`, `lib/blocs/`, `lib/providers/`, `lib/controllers/`, `lib/repositories/`.
  - **Constants / theme**: `lib/constants/`, `lib/theme/`, often files like `app_colors.dart`, `theme.dart`, `constants.dart`.

- **`ios/Runner/Info.plist`** — display name (`CFBundleDisplayName`), bundle ID (`CFBundleIdentifier`), and any platform-specific descriptions (camera/photo/location usage strings often hint at features).

- **`README.md`, `CHANGELOG.md`, `docs/`** — written marketing/feature copy, recent updates, what the team is highlighting.

- **`assets/`** — images, fonts, animations (Lottie), icons. Cross-reference the `flutter:` section of `pubspec.yaml` for declared assets.
...
**Tip — Flutter screenshot CLI**: While the app is running on the iOS Simulator (`flutter run`), the user can capture the current frame with `flutter screenshot --out path/to/file.png` instead of using the Simulator's Cmd+S shortcut. The CLI captures the rendered Flutter frame straight to a file, which is handy for scripting or when iterating through many screens. Either method works — the resulting PNG flows through the same pipeline.
...
1. **Analyse the codebase** — check the Flutter theme and colour definitions:
   - `MaterialApp` / `CupertinoApp` `theme:` and `darkTheme:` properties (typically in `lib/main.dart` or `lib/app.dart`) — look at `colorScheme`, `primaryColor`, `primarySwatch`, `seedColor`
   - Dedicated colour files such as `lib/constants/app_colors.dart`, `lib/theme/colors.dart`, `lib/colors.dart`
   - Widely-used `Color(0xFF...)` literals or `MaterialColor` definitions
   - Custom `ThemeData` / `ThemeExtension` definitions
